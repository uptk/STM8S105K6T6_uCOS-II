...
OS_STK *OSTaskStkInit (void (*task)(void *p_arg), void *p_arg, OS_STK *ptos, INT16U opt)
{
	...
	/* 函数指针压栈 PC为24位*/
    *pstk--  = (INT8U)(tmp & 0xFF);       //PCL
    tmp         >>= 8;
    
    *pstk--  = (INT8U)(tmp & 0xFF);       //PCH
    tmp         >>= 8;
    
    *pstk--  = (INT8U)(tmp & 0xFF);       //PCE

          
    *pstk--  = (INT8U)0x01;               //YL
    *pstk--  = (INT8U)0x02;               //YH
    
    *pstk--  = ((INT16U)p_arg)&0XFF;       //XL
    *pstk--  = (((INT16U)p_arg)>>8)&0XFF;  //XH
    
    *pstk--  = (INT8U)0x05;                //A
    *pstk--  = (INT8U)0x20;                //CC

    return ((OS_STK *)pstk);			   // 返回指向栈顶的指针
}
...
*/
#if 0
@far void  OS_CPU_SysTickHandler (void)
{
    OS_CPU_SR  cpu_sr;
